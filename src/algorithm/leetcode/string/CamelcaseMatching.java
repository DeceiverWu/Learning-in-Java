...
public class CamelcaseMatching {
    ...
    public List<Boolean> camelMatch(String[] queries, String pattern) {
        ...
        for (String query : queries) {
            Boolean isMatch = isMatch(query.toCharArray(), ptn);
            res.add(isMatch);
        }
    ...
    public Boolean isMatch(char[] query, char[] ptn) {
        int pIndex = 0;
        for (int i = 0; i < query.length; i++) {
            if (pIndex < ptn.length && query[i] == ptn[pIndex]) {
                pIndex++;
            }else if (query[i] >= 'A' && query[i] <= 'Z') {
                return false;
            }
        }
        return pIndex == ptn.length;
    }
